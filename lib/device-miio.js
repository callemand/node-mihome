...
module.exports = class MiioDevice extends EventEmitter {
  ...
  constructor({
    id, address, token, protocol, refresh, version = VERSION_1
  }) {
    super();
    this.id = String(id);
  ...
  async loadProperties(props) {
    try {
      ...
      if(this.version === VERSION_2){
        const getPropertiesResult = await this.getProperties(['coucou']);
        if(getPropertiesResult.length === 1){
          result = getPropertiesResult[0];
        }else{
  ...
  async miioGetProperties(props) {
    const result = await this.send((this.version === VERSION_1) ? 'get_prop': 'get_status', props, {
      retries: 1,
    });
    return result;
  }
